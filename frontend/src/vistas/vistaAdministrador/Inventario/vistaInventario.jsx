...
import ModalMovimientosInventario from '../../../componentes/componentesAdmin/modales/modalMovimientoPeps/Ueps.jsx';
...
export default function VistaInventario() {
  ...
  // Estados para el modal de movimientos PEPS/UEPS
  ...
  const abrirModalMovimientos = (producto) => {
  ...
  const handleProductoAgregado = async () => {
    await cargarProductos();
    setMostrarFormularioAgregar(false); // Colapsar el formulario después de agregar
  ...
  // Función para obtener el color del chip según el estado del stock
    ...
    <Box sx={{ minHeight: '100vh', backgroundColor: '#f5f7fa', p: { xs: 2, md: 4 } }}>
      ...
      <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Box sx={{ display: 'flex', gap: 2 }}>
          <Button
            variant={mostrarFormularioAgregar ? "outlined" : "contained"}
            startIcon={mostrarFormularioAgregar ? <KeyboardArrowUpIcon /> : <AddIcon />}
            ...
            sx={{
              ...
              px: 3,
              py: 1.5,
              backgroundColor: mostrarFormularioAgregar ? 'transparent' : '#27ae60',
              borderColor: '#27ae60',
              color: mostrarFormularioAgregar ? '#27ae60' : 'white',
              '&:hover': {
                backgroundColor: mostrarFormularioAgregar ? 'rgba(39, 174, 96, 0.08)' : '#1e8449',
              },
            }}
          >
            {mostrarFormularioAgregar ? 'Ocultar Formulario' : 'Agregar Producto'}
          </Button>
          <Button
            variant="outlined"
      ...
      <Collapse in={mostrarFormularioAgregar} timeout={500}>
        <Box sx={{ mb: 4 }}>
      ...
      {/* Modal de Movimientos PEPS/UEPS */}
      ...
      {!cargando && !error && productos.length === 0 && (
        <Fade in={!cargando && productos.length === 0}>
          <Card
            elevation={2}
            sx={{
              borderRadius: 3,
                      ...
                      >
                        ...
                        <TableCell sx={{ py: 1 }}>
                          <Box sx={{ display: 'flex', gap: 0.5 }}>
                            <Tooltip title="Ver movimientos PEPS/UEPS" arrow>
                              <IconButton
                                size="small"
                                color="info"
                                onClick={() => abrirModalMovimientos(producto)}
                                sx={{
                                  '&:hover': {
